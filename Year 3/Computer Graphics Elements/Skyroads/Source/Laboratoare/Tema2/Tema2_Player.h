...
namespace Tema2_Player
{
	...
	*/
	...
	public:
		...
		int currentLane = 0, direction = 0, health = healthMax;
		bool isFalling = false, gearLock = false, isJumping = false, invulnerable, infiniteFuel;

		VehicleObject() : Tema2_GameObject("vehicle", "VehicleShader", 0.25, -0.75)
		{
			sX = sY = sZ = 0.7;
			y = platformY;
			SetGear(gearMin);
		...
		*/
		void SetGear(int newGear)
		{
			...
			vR = -4 * (forwardGear / (gearMax - gearMin)) * M_PI;
		}
		...
		*/
		...
		void OnDeath()
		{
			...
			cout << "Longest Distance Travelled: " << highScore << endl;
			healthDisplay->sX = 0;
		}
